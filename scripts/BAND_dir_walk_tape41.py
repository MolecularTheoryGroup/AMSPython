    ...
    for current_dir, subdirs, files in os.walk(root_dir):
        ...
        if band_rkf_files:
            ...
            for rkf_file in band_rkf_files:
                ...
                if path_parts:
                    output_filename = '_'.join(path_parts) + '.t41'
                else:
...
def execute_band_commands(file_mappings, grid="fine", spacing=0.05, extend=4.2,
                          atom_inds=None, atom_types=None, length_unit="bohr", grid_save=False,
                          density_plot_options=None):
    """
    ...
    for original_path, output_path in file_mappings:
        ...
        # Build the BAND input with Restart block
        band_input = f"""Restart
  File {original_path}
  DensityPlot Yes
End
...
"""
        ...
        try:
            ...
            input_file = temp_dir / "band_input.in"
            with open(input_file, 'w') as f:
                f.write(band_input)
            
            # Execute BAND in the temporary directory
            result = subprocess.run(
                ["$AMSBIN/band < band_input.in"],
                cwd=str(temp_dir),
                input=band_input,
                text=True,
                shell=True,
            ...
            # Move the generated TAPE41 to the desired output location
            tape41_src = temp_dir / "TAPE41"
            if tape41_src.exists():
                shutil.move(str(tape41_src), output_path)
                print(f"Success: {output_path}")
            else:
                print(f"Error: TAPE41 not found at {tape41_src}")
...
if __name__ == "__main__":
    ...
    execute_band_commands(
        ...
        spacing=0.05,
        extend=4.2,
        density_plot_options=['rho(fit)', 'vxc[rho]']
    )
    ...
    #
    # Custom grid with all atoms:
    # execute_band_commands(file_mappings, grid="custom", spacing=0.05, extend=4.2)
    #
    # Restrict grid to specific atoms by 1-based input-order index:
    # execute_band_commands(file_mappings, grid="custom", atom_inds=[1, 5, 6])
    #
    # Restrict grid to specific element types:
    # execute_band_commands(file_mappings, grid="custom", atom_types=["Fe", "S"])
    #
