...
def amsreport(f, sec, var):
    ...
    try:
        ...
        if "not found" in out or len(out) == 0:
            raise RuntimeError(f"Variable {var} not found in section {sec} for file {f}")
...
def extract_bond_paths(file_path, cp_coords):
    ...
    num_bp_steps = [int(i) for i in amsreport(file_path, 'Properties', 'BP step number').split()]
    ...
    for path_idx in range(num_bps):
        ...
        for step_num in range(num_bp_steps[path_idx]):
            bond_path['coords'].append([float(bp_props[path_idx + i * group_len + step_num * num_bps]) * BOHR_TO_ANG for i in range(3)])
            bond_path['density'].append(float(bp_props[path_idx + 3 * group_len + step_num * num_bps]))
            bond_path['grad'].append([float(bp_props[path_idx + (4 + i) * group_len + step_num * num_bps]) for i in range(3)])
            bond_path['hessian'].append([float(bp_props[path_idx + (7 + i) * group_len + step_num * num_bps]) for i in range(6)])
            
            points.append({'coords': bond_path['coords'][-1], 'density': bond_path['density'][-1], 'grad': bond_path['grad'][-1], 'hessian': bond_path['hessian'][-1]})
        ...
        for i in range(1, len(bond_path['coords'])):
            length += sqrt(sum((bond_path['coords'][i][j] - bond_path['coords'][i-1][j])**2 for j in range(3)))
        bond_path['length'] = length
        
        # Now compute curvature of the bond path (start-end point distance / length)
        curvature = sqrt(sum((bond_path['coords'][-1][i] - bond_path['coords'][0][i])**2 for i in range(3))) / length
        ...
        curvature_angle = angle_between(np.array(bond_path['coords'][1]) - np.array(bond_path['coords'][0]), np.array(bond_path['coords'][-1]) - np.array(bond_path['coords'][-2]))
...
def parse_cp_info(file_path, cp_list_str=None, combine_degenerate_cps=True, degenerate_cp_cutoff_diff=1e-6):        
    cp_data = []
    
    # Get CP properties
    num_cps = int(amsreport(file_path, 'Properties', 'CP number of'))
    cp_type_codes = [int(float(i)) for i in amsreport(file_path, 'Properties', 'CP code number for (Rank,Signatu').split()]
    cp_types = [cp_type_code_map[cp] for cp in cp_type_codes]
    cp_coords = [float(i)* BOHR_TO_ANG for i in amsreport(file_path, 'Properties', 'CP coordinates').split()]
    ...
    cp_hessian = [float(i) for i in amsreport(file_path, 'Properties', 'CP density Hessian at').split()]
    cp_hessian = [cp_hessian[i:i+5*num_cps+1:num_cps] for i in range(num_cps)] # 6 values per CP: XX, XY, XZ, YY, YZ, ZZ
      ...
      for cp_idx in range(num_cps):
        ...
        for cp_idx2 in range(cp_idx+1, num_cps):
          if cp_idx2 in degenerate_cp_reverse_map:
            continue
          if abs(float(cp_density[cp_idx]) - float(cp_density[cp_idx2])) < degenerate_cp_cutoff_diff and cp_types[cp_idx] == cp_types[cp_idx2]:
            degenerate_cp_count[cp_idx] += 1
    ...
    for cp_idx in range(num_cps):
      ...
      if cp_types[cp_idx] != 'nuclear':
          ...
          if cp_types[cp_idx] in ['bond', 'ring']:
            hessian6 = cp_hessian[cp_idx]
            hessian = [[hessian6[0], hessian6[1], hessian6[2]], [hessian6[1], hessian6[3], hessian6[4]], [hessian6[2], hessian6[4], hessian6[5]]]
            eig = np.linalg.eig(np.array(hessian))
            ev = eig[0]
            ...
            cp_dict.update({
                ...
                'EIGENVALUES OF HESSIAN MATRIX': ev.tolist(),
                'EIGENVECTORS (ORTHONORMAL) OF HESSIAN MATRIX (COLUMNS)': eig[1].tolist(),
                'Laplacian': laplacian,
                'Theta': theta,
                'Phi': phi,
            })
            if cp_types[cp_idx] == 'bond' and cp_idx1 in bp_data:
              cp_dict.update({
                  ...
                  'Bond Path Curvature (length ratio)': bp_data[cp_idx1]['curvature (length ratio)'],
                  'Bond Path Curvature (end angle)': bp_data[cp_idx1]['curvature (end angle)'],
              })
...
def main(args=None):
    ...
    parser.add_argument('input_file_or_folder', type=str, help='Path to the input band .rkf file, or a folder that will "walked" through, processing all band.rkf files in any of the subfolders.')
    parser.add_argument('--cps', type=str, default=None, help='Comma-delimited list of CPs to include (e.g., "1,3,5-8,11"; See AMSview or output file for CP numbering). If not specified, all CPs will be included.')
    parser.add_argument('--keep-degenerate-cps', action='store_true', help='Keep degenerate CPs as separate lines in the output CSV file. Default is to combine them, showing the number of degenerate CPs in a column. This option overrides --cps, so if the CP list includes degenerate CPs, those CPs will be replaced with the lowest index non-degenerate CP.')
    ...
    if os.path.isfile(args.input_file_or_folder):
        if args.input_file_or_folder.endswith("band.rkf"):
          cp_data = parse_cp_info(args.input_file_or_folder, args.cps, not args.keep_degenerate_cps, degenerate_cp_cutoff_diff=1e-6)
        ...
        for i,path in enumerate(paths):
            ...
            path_prefix = path_minus_filename.replace(common_path, "").replace("/", "_")[1:]
            ...
            # print(path_prefix)
            try:
                cp_data = parse_cp_info(path, args.cps, not args.keep_degenerate_cps, degenerate_cp_cutoff_diff=1e-6)
